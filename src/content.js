// Static text content for JJJUICE website
export const content = {
  meta: {
    title: "JJJUICE | Bald Erhältlich",
    language: "de"
  },
  
  nav: {
    brand: "JJJUICE SALSA CO."
  },
  
  hero: {
    headline: ["SPÜR", "DAS", "FEUER"],
    subheadline: "Authentische mexikanische Salsa – mit Tradition, Liebe und der perfekten Schärfe.",
    ctaButton: "Benachrichtigen",
    logoAlt: "JJJUICE Logo"
  },
  
  comingSoon: {
    sectionTitle: "Bald Erhältlich",
    countdown: {
      days: "Tage",
      hours: "Stunden",
      minutes: "Minuten",
      seconds: "Sekunden"
    },
    signup: {
      placeholder: "Deine E-Mail-Adresse",
      button: "Dabei",
      note: "Sei der Erste, der probiert. Kein Spam, nur Salsa.",
      alertMessage: "¡Gracias! Wir informieren dich als Erstes."
    },
    features: [
      {
        title: "Kleine Mengen",
        description: "Handgefertigt in kleinen Mengen für kompromisslose Qualität und Geschmack."
      },
      {
        title: "Frische Zutaten",
        description: "Frisch zubereitet mit Zutaten aus München."
      },
      {
        title: "Mit Liebe Gemacht",
        description: "Die gleiche Salsa, die ich für Freunde und Familie zubereite."
      }
    ]
  },
  
  salsas: {
    sectionTitle: "Unsere Salsas",
    products: [
      {
        name: "Mad Hot Salsa",
        tagline: "Klassisch. Feurig. Unvergesslich.",
        description: "Feuergeröstete Chilis, reife Tomaten, ordentlich Schärfe. Das Rezept kommt aus unseren Jahren in Austin und West-Texas — dort haben wir gelernt, wie Salsa schmecken muss. Rauchig, direkt, ohne Kompromisse. Handgemacht in München, in kleinen Chargen.",
        ingredients: "Tomaten, Chipotle, Knoblauch, Zwiebel, Koriander, Limette, Salz",
        image: "/mad-hot.png",
        imageAlt: "Mad Hot Salsa"
      },
      {
        name: "Nice'n'Mild Salsa",
        tagline: "Frisch. Herb. Unwiderstehlich.",
        description: "Dieselben gerösteten Tomaten und Chilis, derselbe Geschmack aus Texas — nur ohne die Schärfe. Mild, rauchig, ausgewogen. Für alle, die gutes Essen gern teilen. Handgemacht in München, in kleinen Chargen.",
        ingredients: "Tomatillos, Jalapeño, Knoblauch, Zwiebel, Koriander, Limette, Salz",
        image: "/nice-n-mild.png",
        imageAlt: "Nice'n'Mild Salsa"
      }
    ]
  },

  footer: {
    brand: "JJJUICE",
    copyright: "© 2026 JJJ GMBH. ALLE RECHTE VORBEHALTEN"
  }
};

// Made with Bob
